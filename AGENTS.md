...
- iOS/Capacitor 작업은 `./run-ios.sh` (**Mac 전용** — xcrun·시뮬레이터 전제):
  API 환경 선택(local/beta/prod — 인자로도 지정 가능) → 빌드 → `cap sync ios` → Xcode 열기.
  local은 로컬 백엔드(localhost:8080)로 붙는 **시뮬레이터 전용 개발 빌드** — 릴리스
  매니페스트 없이 웹 동작으로 강등된다(`app/_lib/native/nativeLocalDev.ts`, fail-closed).
