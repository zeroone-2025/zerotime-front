...
test('게스트가 학교를 빠르게 여러 번 전환해도 게시판 목록이 정상 유지된다', async ({ page }) => {
  ...
  const schools = ['전남대', '경북대', '충남대', '충북대', '전북대', '충북대'];
  ...
  await expect(trigger).toContainText('충북대학교');

  // 필터 화면으로 이동해 실제 게시판 목록이 충북대 기준으로 뜨는지 확인
  await page.goto('/filter');
  await expect(page.locator('.animate-spin')).toHaveCount(0, { timeout: 15_000 });
  await expect(page.getByText('컴퓨터과학과').first()).toBeVisible({ timeout: 10_000 });
