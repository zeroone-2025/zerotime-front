...
1. **환경 결정** — 무인자는 `local` (dev-api는 앱에서 쓰지 않습니다 — 웹 전용)
...
- 릴리스 매니페스트 없이 웹 동작으로 강등되는 개발 전용 빌드
  (`app/_lib/native/nativeLocalDev.ts` — 로컬 http origin만 허용)
- **게스트 플로우 전용** — 로그인·푸시 등 네이티브 인증이 필요한 기능은 동작하지
  않습니다 (네이티브 인증은 릴리스 런타임의 코디네이터 전제, OAuth 콜백도 웹뷰 밖).
  이 앱은 게스트 우선 설계라 공지·필터 등 대부분의 화면은 로그인 없이 확인 가능합니다.
