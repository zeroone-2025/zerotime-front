...
- iOS/Capacitor 작업은 **Mac 전용** 스크립트를 쓴다 (`ipconfig`·BSD sed·xcrun 전제 — Linux 불가):
  - `./run-dev.sh` — .env.local 준비(로컬 IP 감지) + 빌드 + `cap sync ios` + 시뮬레이터 + dev 서버
  - `./run-ios.sh` — API 환경(local/dev/prod) 선택 → 빌드 → `cap sync ios` → Xcode 열기.
...
- `.env.local`은 `.env.sample`을 복사해 만든다 (`YOUR_LOCAL_IP` 플레이스홀더를 스크립트가 치환 —
  하네스 `../dev.sh front`는 localhost로, `run-dev.sh`는 감지한 로컬 IP로).
