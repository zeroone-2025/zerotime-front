...
`app/_lib/constants/boards.ts`의 `BOARD_MAP`(활성 게시판 120여 개 — Pending 게시판은 크롤러
...
npm run test:run         # Vitest 유닛 테스트 1회 실행 (app/**/*.test.ts(x), e2e/ 제외)
                         #   ⚠️ `npm test`는 watch 모드라 끝나지 않는다 — 에이전트는 test:run
...
- iOS/Capacitor 작업은 `./run-ios.sh` (**Mac 전용** — xcrun·시뮬레이터 전제):
  무인자는 개발 기본값 local, 릴리스 검증은 인자(beta/prod), 종료는 `down`(시뮬레이터 종료).
  local은 로컬 백엔드(localhost:8080)로 붙는 **시뮬레이터 전용·게스트 플로우 전용** 개발
  빌드 — 릴리스 매니페스트 없이 웹 동작으로 강등되며(`app/_lib/native/nativeLocalDev.ts`,
  fail-closed) 로그인·푸시는 동작하지 않는다(검증은 beta).
  네이티브 릴리스 plane은 beta(TestFlight)/prod(App Store) 둘뿐 — dev-api는 앱에서 안 쓴다.
  상세: `docs/ios-dev-guide.md`, 스토어 배포는 `IOS_RELEASE_CHECKLIST.md`
- Android/Capacitor 작업은 `./run-android.sh` (Android SDK 전제 — 없으면 설치 안내 후 종료):
  API 환경 선택(인자로도 지정 가능, local/dev는 beta plane으로 강제) → 빌드 → `cap sync android`
  → 에뮬레이터 기동 → debug flavor 설치·실행. ⚠ out/은 플랫폼별 빌드라 iOS와 공유 불가 —
  동시 테스트 순서는 스크립트 상단 주석 참조.
...
| 변수 | 대상 | 기본값 |
|---|---|---|
| `NEXT_PUBLIC_API_BASE_URL_WEB` | 웹(및 빌드타임 프리렌더)·네이티브 로컬 개발 빌드 | `http://localhost:8080` |
| `NEXT_PUBLIC_API_BASE_URL_NATIVE` | 네이티브 릴리스 매니페스트의 api_origin 입력 (plane과 일치 강제) | `run-ios.sh`가 주입 |

- 네이티브 릴리스 런타임의 실제 API origin은 검증된 매니페스트에서 나온다
  (`mobileRelease.ts`) — NATIVE 변수는 그 검증 입력이며 beta/prod plane의 도메인과
  일치하지 않으면 빌드가 실패한다. 네이티브 로컬 개발 빌드(`nativeLocalDev.ts`)는
  매니페스트 없이 WEB 변수(로컬 http origin만 허용)를 쓴다.
- `.env.local`은 `.env.sample`을 복사해 만든다 (`YOUR_LOCAL_IP` 플레이스홀더를 `run-dev.sh`가
  localhost로 치환). 환경별 URL은 `run-ios.sh`가 빌드 시점에 환경변수로 주입한다 —
