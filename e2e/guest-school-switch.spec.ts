...
const BOARD_FIXTURES = {
  ...
  충북대: [{ board_code: 'cbnu_dept_computer_science', name: '컴퓨터과학과', school: '충북대', category: '학과', default_subscribe: true }],
  부산대: [{ board_code: 'pnu_dept_cse', name: '컴퓨터공학전공', school: '부산대', category: '학과', default_subscribe: true }],
} as const;
...
test('게스트가 학교를 빠르게 여러 번 전환해도 게시판 목록이 정상 유지된다', async ({ page }) => {
  ...
  const schools = ['전남대', '경북대', '충남대', '충북대', '전북대', '부산대'];
  for (const school of schools) {
    await trigger.click();
    // exact: true — MOCK_USER_STATS.school이 '전북대학교'로 고정 목킹돼 있어
    // 상단 통계 배너("벌써 N명의 전북대학교 학생이...") 버튼과 이름이 겹친다.
    await page.getByRole('button', { name: FULL_NAME[school], exact: true }).click({ force: true });
  ...
  await expect(trigger).toContainText('부산대학교');

  // 필터 화면으로 이동해 실제 게시판 목록이 부산대 기준으로 뜨는지 확인
  await page.goto('/filter');
  await expect(page.locator('.animate-spin')).toHaveCount(0, { timeout: 15_000 });
  await expect(page.getByText('컴퓨터공학전공').first()).toBeVisible({ timeout: 10_000 });
